...
Cette pratique couvre essentiellement les phase de *planitifcation* et de *programmation* (code) dans un cycle de vie DevOps et on peut y inclure des mécanismes de contrôle des versions (comme *Git* par exemple).

Le mécanisme de contrôle des version est une première réponse naturelle à l'ajout de la sécurité dans le DevOps car il permet de tracer toutes les modifciations apportées dans le temps, au code surveillé. Bien évidemment, ce n'est pas suffisant.
...
On le verra un peu plus tard mais, cette phase est très importante pour amélirer la sécurité du DevOps, c'est par elle qu'une application est intégrée et ainsi il est très important de pouvoir s'assurer de sa correcte intégration (exemple, *les rapports*).

### La livraison continue

La livraison continue, commme son nom l'indique, permet la publication des modifications du code après la phase de tests. Très souvent c'est envoyé dans un environnement intermédiaire (environnement situé avant la production), pour s'assurer que l'application ne pose pas de problème une fois en production.

Ces environnements sont régulièrement appelés *préproduction*, ce terme est essentiel car il sera réutilisait par la suite, c'est dans cet environnement que tout sera joué par la suite.
...
Très régulièrement, c'est dans ce genre de contexte que l'on va retrouver les langages d'y "*déclaratif*" pour déclarer l'état souhaité de notre infrastructure.
